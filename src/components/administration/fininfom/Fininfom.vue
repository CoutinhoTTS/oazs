    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-file-code-o "></i><span>申请人：{{item.realname}}</span></div>
            </div>
            <div>
              <div><span>申请日期:</span><span>{{item.applydt}}</span></div>
              <div><span>收款人:</span><span>{{item.fullname}}</span></div>
            </div>
            <div>
              <div><span>收款账号:</span><span>{{item.cardid}}</span></div>
              <div><span>开户行:</span><span>{{item.openbank}}</span></div>
            </div>
            <div>
              <div><span>报销金额:</span><span>{{item.money}}</span></div>
              <div >
...
import {fininfomlist} from '../../../api/Api'
export default {
  name: 'Fininfom',
  components: {
    NavBar,

      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>收款人:</div><div>{{list.fullname}}</div></div>
            <div><div>收款账号:</div><div>{{list.cardid}}</div></div>
        ...
        <van-tab title="报销明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              <div>
                <div><span>发生日期:</span><span>{{item.sdt}}</span></div>
                <div><span>报销项目:</span><span>{{item.name}}</span></div>
              </div>
              <div>
                <div><span>金额(元):</span><span>{{item.money}}</span></div>
                <div><span>说明:</span><span>{{item.sm}}</span></div>
              </div>
            </div>
          </div>
        </van-tab>
...
import {fininfomshow, getreadsandlog} from '../../../api/Api'
export default {

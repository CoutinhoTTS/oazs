  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
          <div class="datailsList">
            ...
            <div><div>付款金额(元):</div><div>{{list.money}}</div></div>
            <div><div>付款方式:</div><div>{{list.paytype}}</div></div>
            <div><div>付款日期:</div><div>{{list.paydt}}</div></div>
            <div><div>收款方名称:</div><div>{{list.receivablesunit}}</div></div>
            <div><div>开户银行:</div><div>{{list.receivablesbank}}</div></div>
            <div><div>收款账号:</div><div>{{list.receivablesnum}}</div></div>
          </div>
          <van-panel title="说明:" >
...
import {paymentshow, getreadsandlog} from '../../../api/Api'
export default {

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-shopping-bag"></i><span>名称：{{item.name}}</span></div>
            </div>
            <div>
              <div><span>编号:</span><span>{{item.num}}</span></div>
              <div><span>保管人:</span><span>{{item.managename}}</span></div>
            </div>
            <div>
              <div><span>签发日期:</span><span>{{item.startdt}}</span></div>
              <div><span>截止日期:</span><span>{{item.enddt}}</span></div>
            </div>
            <div>
              <div><span>分类:</span><span>{{item.type}}</span></div>
              <div><span>存放地:</span><span>{{item.place}}</span></div>
            </div>
          </div>
        </div>
      </van-list>
...
import {managefilelist} from '../../../api/Api'
export default {
  name: 'Certcostsettle',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      managefilelist(params).then((res) => {

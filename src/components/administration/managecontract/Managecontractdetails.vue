  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>客户名称:</div><div>{{list.cusname}}</div></div>
        <div><div>合同金额(元):</div><div>{{list.money}}</div></div>
        <div><div>结算金额(元):</div><div>{{list.settle_money}}</div></div>
        <div><div>保管人:</div><div>{{list.managename}}</div></div>
        ...
        <div><div>分类:</div><div>{{list.type}}</div></div>
        <div><div>存放地:</div><div>{{list.place}}</div></div>
      </div>
      <van-panel title="说明:" >
...
import {managecontractshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    managecontractshow(params).then((res) => {

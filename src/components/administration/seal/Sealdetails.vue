  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        <div><div>单号:</div><div>{{list.name}}</div></div>
        <div><div>申请人:</div><div>{{list.num}}</div></div>
        <div><div>申请日期:</div><div>{{list.type}}</div></div>
        <div><div>收款人:</div><div>{{list.bgname}}</div></div>
        <div class="imgs_"><div>相关图片:</div><div><img class="imgs" v-for="(img, index) in imageList" v-lazy="img" :key="index"></div></div>
        <div><div>签发日期:</div><div>{{list.startdt}}</div></div>
...
import {sealshow, getreadsandlog} from '../../../api/Api'
export default {
  name: 'Sealdetails',
  components: {
    NavBar,

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-file-excel-o "></i><span>申请人：{{item.realname}}</span></div>
            </div>
            <div>
            ...
            <div>
              <div><span>印章证照:</span><span>{{item.sealname}}</span></div>
              <div><span>是否外带:</span><span v-if="item.isout === 1">是</span><span v-else>否</span></div>
            </div>
            <div>
              <div></div>
              <div>
...
import {sealapllist} from '../../../api/Api'
export default {
  name: 'Sealapl',
  components: {
    NavBar,

      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>调出仓库:</div><div>{{list.depotname}}</div></div>
            <div><div>单号:</div><div>{{list.sericnum}}</div></div>
            <div v-if="list.state === 0"><div>出库状态:</div><div>待出库</div></div>
            <div v-else-if="list.state === 1"><div>出库状态:</div><div>已全部出库</div></div>
            <div v-else-if="list.state === 2"><div>出库状态:</div><div>部分出库</div></div>
        ...
        <van-tab title="物品信息">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              <div>
                <div><span>物品:</span><span>{{item.name}}</span></div>
                <div><span>调拨数量:</span><span>{{item.count}}</span></div>
              </div>
            </div>
          </div>
        </van-tab>
...
import {goodmcallshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    goodmcallshow(params).then((res) => {
      if (res.data.code === 2000) {
        this.list = res.data.data.data
        this.conlist = res.data.data.data.goods

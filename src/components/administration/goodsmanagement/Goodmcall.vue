    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-save "></i><span>申请人：{{item.realname}}</span></div>
            </div>
            <div>
            ...
            <div>
              <div><span>调出仓库:</span><span>{{item.depotname}}</span></div>
...
import {goodmcalllist} from '../../../api/Api'
export default {
  name: 'Goodmcall',
  components: {
    NavBar,

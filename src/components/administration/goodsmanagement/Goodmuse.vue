    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-paste "></i><span>申请人：{{item.realname}}</span></div>
            </div>
            <div>
            ...
            <div>
              <div><span>领用的物品:</span><span>{{item.goodsname}}</span></div>
              <div><span>出库状态:</span><span>{{item.state_name}}</span></div>
            </div>
            <div>
              <div></div>
              <div>
...
import {goodmuselist} from '../../../api/Api'
export default {
  name: 'Goodmuse',
  components: {
    NavBar,

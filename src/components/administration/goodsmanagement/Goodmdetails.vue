      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>供应商:</div><div>{{list.custname}}</div></div>
            <div><div>优惠价格:</div><div>{{list.discount}}</div></div>
            <div><div>采购金额:</div><div>{{list.money}}</div></div>
          ...
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              <div>
                ...
                <div><span>采购数量:</span><span>{{item.count}}</span></div>
              </div>
              <div>
...
import {goodmshow, getreadsandlog} from '../../../api/Api'
export default {
  name: 'Goodmdetails',
  components: {
    NavBar,

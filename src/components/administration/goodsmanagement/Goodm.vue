    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-th"></i><span>申请人：{{item.realname}}</span></div>
            </div>
            <div>
              <div><span>申请人部门:</span><span>{{item.name}}</span></div>
              <div><span>申请日期:</span><span>{{item.applydt}}</span></div>
            </div>
            <div>
              <div><span>供应商名称:</span><span>{{item.custname}}</span></div>
              <div><span>优惠价格:</span><span>{{item.discount}}</span></div>
            </div>
            <div>
              <div><span>采购金额:</span><span>{{item.money}}</span></div>
              <div><span>入库状态:</span><span>{{item.state_name}}</span></div>
            </div>
            <div>
             <div></div>
              <div>

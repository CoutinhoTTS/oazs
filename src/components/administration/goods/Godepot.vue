    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-home"></i><span>仓库名称：{{item.depotname}}</span></div>
            </div>
            <div>
              <div><span>仓库地址:</span><span>{{item.depotaddress}}</span></div>
              <div><span>仓库管理员:</span><span>{{item.cgname}}</span></div>
            </div>
            <div>
              <div><span>排序号:</span><span>{{item.sort}}</span></div>
              <div><span>仓库编号:</span><span>{{item.depotnum}}</span></div>
            </div>
          </div>
        </div>
      </van-list>
...
import {godepotlist} from '../../../api/Api'
export default {
  name: 'Godepot',
  components: {
    NavBar,

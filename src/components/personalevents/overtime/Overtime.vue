      ...
      <van-tab :title="titles[0]" class="tabs_">
        ...
        <van-cell-group>
          ...
          <van-radio-group v-model="radio">
            <div style="font-size: 0.9rem;font-weight: bolder;padding-left: 0.5rem"><span style="color: red">*</span>加班兑换:</div>
            <van-cell-group>
              <van-cell title="调休" clickable @click="radio = '0'">
                <van-radio slot="right-icon" name="0" />
              </van-cell>
              <van-cell title="加班费" clickable @click="radio = '1'">
...
import {mysheet, uploadfile, addkqinfojb, addkqinfojbpost, systembaseusershow} from '../../../api/Api'
export default {
  name: 'Leave',
  components: {
    NavBar,
  ...
  created () {
    ...
    // 初始化请假数据
    addkqinfojb({token: this.token, modenum: this.modenum}).then((res) => {
      if (res.data.code === 2000) {
  ...
  methods: {
    ...
    gitqingjia () {
      if (!this.leaveperson.id) {
        ...
        this.$toast('请选择加班开始时间')
        return false
      } else if (!this.evalue) {
        this.$toast('请选择加班结束时间')
        return false
      }
      let param = {
        token: this.token,
        ...
        jiafee: this.jiabanfei,
        isturn: 1,
        file: this.file_id,
        cho_checkid: '',
        cho_checkname: ''
      }
      addkqinfojbpost(param).then((res) => {
        if (res.data.code === 2000) {
          this.$toast('提交成功！')
          setTimeout(() => {
            this.$router.go(0)

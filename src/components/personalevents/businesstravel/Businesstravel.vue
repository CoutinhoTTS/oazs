  ...
  <div style="margin-bottom: 3.125rem">
    ...
    <van-tabs v-model="active" swipeable class="tabs">
      <van-tab :title="titles[0]" class="tabs_">
        ...
        <div class="leavetime">外出出差</div>
        <!--时间选择-->
        <van-cell-group>
          <van-radio-group v-model="radio">
            <div style="font-size: 0.9rem;font-weight: bolder;padding-left: 0.5rem"><span style="color: red">*</span>类型:</div>
            <van-cell-group>
              <van-cell title="外出" clickable @click="radio = '0'">
                <van-radio slot="right-icon" name="0" />
              </van-cell>
              <van-cell title="出差" clickable @click="radio = '1'">
...
import {mysheet, uploadfile, addkqout, addkqoutpost, systembaseusershow} from '../../../api/Api'
export default {
  name: 'Leave',
  components: {
    NavBar,
  ...
  created () {
    ...
    // 初始化请假数据
    addkqout({token: this.token, modenum: this.modenum}).then((res) => {
      if (res.data.code === 2000) {
        this.leaveperson.name = res.data.data.realname
  ...
  methods: {
    ...
    gitqingjia () {
      if (!this.leaveperson.id) {
        ...
        this.$toast('请选择外出开始时间')
        return false
      } else if (!this.evalue) {
        this.$toast('请选择回岗时间')
        return false
      } else if (!this.resons_) {
        this.$toast('请填写外出理由')
        return false
      }
      var s
      ...
      let param = {
        ...
        applyname: this.leaveperson.name,
        outtime: this.svalue,
        intime: this.evalue,

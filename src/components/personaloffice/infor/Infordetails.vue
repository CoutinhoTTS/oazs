  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        <div><div>标题:</div><div>{{list.title}}</div></div>
        <div><div>[通知公告]:</div><div>{{list.optdt}}</div></div>
      </div>
      <van-panel title="内容:" >
        <div class="explian" v-html="list.content"></div>
      </van-panel>
      <div class="datailsList">
        <div><div>发件人:</div><div>{{list. zuozhe}}</div></div>
        <div><div>时间:</div><div>{{list.indate}}</div></div>
      </div>
      <van-panel title="相关地址:" >
        <div class="explian">{{list.url}}</div>
      </van-panel>
      <!--文件-->
      <File ref="files"></File>
      <!--审核，游览-->
...
import {inforshow, getreadsandlog} from '../../../api/Api'
export default {
  name: 'Goodmdetails',
  components: {
    NavBar,

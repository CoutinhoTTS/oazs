  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        <div><div>主题:</div><div>{{list.title}}</div></div>
        <div><div>发送人:</div><div>{{list.sendname}}</div></div>
        <div><div>收件人:</div><div>{{list.recename}}</div></div>
        <div><div>抄送给:</div><div>{{list.ccname}}</div></div>
        <div><div>发送时间:</div><div>{{list.senddt}}</div></div>
      </div>
      <van-panel title="内容:" >
        <div class="explian_" v-html="list.content"></div>
      </van-panel>
      <!--文件-->
      <File ref="files"></File>
      <!--审核，游览-->
...
import {emailmshow, getreadsandlog} from '../../../api/Api'
export default {

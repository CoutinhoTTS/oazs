  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>收取单位:</div><div>{{list.collect}}</div></div>
        <div><div>收取单位:</div><div>{{list.collect}}</div></div>
        <div><div>开户行:</div><div>{{list.open_bank}}</div></div>
        <div><div>账号:</div><div>{{list.acc_number}}</div></div>
        <div><div>保证金金额(元):</div><div>{{list.money}}</div></div>
        <div><div>交款日期:</div><div>{{list.paydt}}</div></div>
        <div><div>预计退款日期:</div><div>{{list.refunddt}}</div></div>
      </div>
      <van-panel title="说明:" >
...
import {bidbondshow, getreadsandlog} from '../../../api/Api'
export default {
  name: 'Bidbonddetails',
  components: {
    NavBar,

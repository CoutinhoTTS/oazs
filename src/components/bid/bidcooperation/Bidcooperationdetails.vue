  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>企业名称:</div><div>{{list.name}}</div></div>
        <div><div>法人:</div><div>{{list.legalperson}}</div></div>
        <div><div>单位地址:</div><div>{{list.address}}</div></div>
        <div><div>联系人:</div><div>{{list.person}}</div></div>
        <div><div>联系电话:</div><div>{{list.tel}}</div></div>
        <div><div>合作方式:</div><div>{{list.type}}</div></div>
        <div><div>合同费用:</div><div>{{list.fee}}</div></div>
        <div><div>费用支付时间:</div><div>{{list.paydt}}</div></div>
      </div>
      <van-panel title="说明:" >
...
import {bidcooperationshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    bidcooperationshow(params).then((res) => {

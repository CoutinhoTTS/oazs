  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>任务名:</div><div>{{list.title}}</div></div>
        <div><div>等级任务:</div><div>{{list.grade}}</div></div>
        <div><div>分配给:</div><div>{{list.dist}}</div></div>
        <div><div>开始时间:</div><div>{{list.startdt}}</div></div>
        <div><div>结束时间:</div><div>{{list.enddt}}</div></div>
        <div><div>督导人员:</div><div>{{list.ddname}}</div></div>
      </div>
      <van-panel title="说明:" >
...
import {bidworkshow, getreadsandlog} from '../../../api/Api'
export default {
  name: 'Bidworkdetails',
  components: {
    NavBar,

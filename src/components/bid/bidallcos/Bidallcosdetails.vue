  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>费用名:</div><div>{{list.name}}</div></div>
        <div><div>金额(万元):</div><div>{{list.money}}</div></div>
        <div><div>付款时间:</div><div>{{list.paydt}}</div></div>
        <div><div>经办人:</div><div>{{list.jingbanname}}</div></div>
...
import {bidcosshow, getreadsandlog} from '../../../api/Api'
export default {

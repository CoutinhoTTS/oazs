  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="certlist">
      <van-list
        ...
        @load="onLoad"
      >
        <div class="certalllist" v-for="(item, index) in list" :key="index" :data-id ="item.id" @click="certtomyopendatails(item.id)">
          <div>
            <div>
          ...
          <div>
            <span> 证书专业:{{item.cert_major_arr}}</span>
            <span style="color: red"> 是否过期:{{item.ware}}</span>
          </div>
        </div>
      </van-list>
    </van-pull-refresh>
...
import {certexpirelist} from '../../../api/Api'
export default {
  name: 'certtomyopen',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      certexpirelist(pramas).then((res) => {
    ...
    onLoad () {
      ...
      certexpirelist(pramas).then((res) => {
        if (res.data.code === 2000) {
          ...
          for (var n = 0; n < res.data.data.cert_type.length; n++) {
            this.columns.push({
              text: res.data.data.cert_type[n].name,
              value: res.data.data.cert_type[n].id
            })
          }

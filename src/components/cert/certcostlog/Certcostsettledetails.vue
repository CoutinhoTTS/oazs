      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>填报日期:</div><div>{{list.applydt}}</div></div>
            <div><div>单号:</div><div>{{list.number}}</div></div>
            <div><div>结算开始日期:</div><div>{{list.startdt}}</div></div>
            <div><div>结算结束日期:</div><div>{{list.enddt}}</div></div>
            <div><div>证书使用次数:</div><div>{{list.cert_num}}</div></div>
            <div><div>结算总金额(元):</div><div>{{list.settlemoney}}</div></div>
            <div><div>分摊总金额(元):</div><div>{{list.sharemoney}}</div></div>
          </div>
          <van-panel title="说明:" >
            <div class="explian">{{list.remark}}</div>
          </van-panel>
        </van-tab>
        <van-tab title="证书使用明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              <div>
                <div><span>使用部门/项目:</span><span>{{item.dpname}}</span></div>
              </div>
              <div>
        ...
        <van-tab title="证书使用统计">
          <div class="conlists">
            <div v-for="(item, index) in conlist_" :key="index">
              <div>
                <div><span>使用部门项目:</span><span>{{item.dpname}}</span></div>
              </div>
              <div>
                <div><span>使用证书次数:</span><span>{{item.num}}</span></div>
                <div><span>使用天数:</span><span>{{item.day}}</span></div>
              </div>
              <div>
                <div><span>使用费用(元):</span><span>{{item.use_money}}</span></div>
                <div><span>分摊比例(%):</span><span>{{item.sharescale}}</span></div>
              </div>
              <div>
                <div><span>分摊金额(元):</span><span>{{item.sharemoney}}</span></div>
...
import {certcostsettleshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    certcostsettleshow(params).then((res) => {
      if (res.data.code === 2000) {
        this.list = res.data.data.data
        // 施工清单
        this.conlist = res.data.data.detail
        this.conlist_ = res.data.data.count

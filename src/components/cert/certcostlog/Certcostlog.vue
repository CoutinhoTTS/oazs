    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="certcontlist">
      ...
      >
        <div v-for="(item, index) in list" :key="index" class="certcontlists" @click="certcontdetails(item.id)">
          <div style="width: 100%">
            <div style="width: 100%"><span>部门或项目:</span><span>{{item.name}}</span></div>
          </div>
          <div>
            <div><span>单号:</span><span>{{item.number}}</span></div>
          </div>
          <div>
            <div><span>使用总费用(元):</span><span>{{item.allmoney}}</span></div>
            <div><span>分摊总费用(元):</span><span>{{item.fenmoney}}</span></div>
          </div>
          <div>
            <div><span>用途:</span><span>{{item.purpose}}</span></div>
...
import {certcostloglist} from '../../../api/Api'
export default {
  name: 'Certcontract',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      certcostloglist(param).then((res) => {

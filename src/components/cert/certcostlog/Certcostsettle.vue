    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="certcontlist">
      ...
      >
        <div v-for="(item, index) in list" :key="index" class="certcontlists" @click="certcontdetails(item.id)">
          <div style="width: 100%">
            <div style="width: 100%"><span>单号:</span><span>{{item.number}}</span></div>
          </div>
          <div>
            <div><span>结算开始日期:</span><span>{{item.startdt}}</span></div>
            <div><span>结算结束日期:</span><span>{{item.enddt}}</span></div>
          </div>
          <div>
            <div><span>证书使用次数:</span><span>{{item.cert_num}}</span></div>
            <div><span>结算总费用(元):</span><span>{{item.settlemoney}}</span></div>
          </div>
          <div>
            <div><span>分摊总金额(元):</span><span>{{item.sharemoney}}</span></div>
...
import {certcostsettlelist} from '../../../api/Api'
export default {
  name: 'Certcontract',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      certcostsettlelist(param).then((res) => {

  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="certcontlist">
      <van-list
        ...
        @load="onLoad"
      >
        <div v-for="(item, index) in list" class="certcontlists" :key="index" @click="Certcosdetails(item.id)">
          <div>
            <div><i class="fa fa-credit-card"></i><span>{{item.contractname}}</span></div>
            <div><span>费用名称:</span><span>{{item.name}}</span></div>
            <div><span>费用类型:</span><span>{{item.type}}</span></div>
          </div>
          <div>
            <div><span>收/付款时间:</span><span>{{item.paydt}}</span></div>
            <div><div v-html="item.status_name">
            </div></div>
          </div>
        </div>
      </van-list>
...
import {certcoslist} from '../../../api/Api'
export default {
  name: 'Certcos',
  components: {
    NavBar,

  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div><span class="requir">*</span>费用名称:</div><div>{{list.name}}</div></div>
        <div><div><span class="requir">*</span>费用类型:</div><div>{{list.type}}</div></div>
        <div><div><span class="requir">*</span>金额(元):</div><div>{{list.money}}</div></div>
        <div><div>付款时间:</div><div>{{list.paydt}}</div></div>
        <div><div><span class="requir">*</span>经办人:</div><div>{{list.jingbanname}}</div></div>
      </div>
      <van-panel title="说明:" >
...
import {certcosshow, getreadsandlog} from '../../../api/Api'
export default {
  name: 'Certcosdetails',
  components: {
    NavBar,

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-newspaper-o"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div>
              <div><span>发票类型:</span><span>{{item.invmethod}}</span></div>
              <div><span>发票抬头:</span><span>{{item.fuunit}}</span></div>
            </div>
            <div>
              <div><span>发票金额:</span><span>{{item.money}}</span></div>
              <div><span>发票号码:</span><span>{{item.number}}</span></div>
            </div>
            <div>
              <div><span>开具时间:</span><span>{{item.dt}}</span></div>
              <div><span>开票单位:</span><span>{{item.shouunit}}</span></div>
            </div>
            <div>
            ...
            <div>
              <div><span>经办人:</span><span>{{item.jingban}}</span></div>
...
import {engininvoicelist} from '../../../api/Api'
export default {
  ...
  methods: {
    ...
    onRefresh () {
      ...
      engininvoicelist(params).then((res) => {

      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>合同编号:</div><div>{{list.setmoney}}</div></div>
            <div><div>合同类型:</div><div>{{list.dealcom}}</div></div>
            <div><div>开始日期:</div><div>{{list.jingban}}</div></div>
        ...
        <van-tab title="租赁明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              <div>
                <div><span>租赁物品:</span><span>{{item.name}}</span></div>
              </div>
              <div>
              ...
              <div>
                ...
                <div><span>进场日期:</span><span>{{item.shi_intodt}}</span></div>
              </div>
              <div>
                <div><span>退场日期:</span><span>{{item.shi_outdt}}</span></div>
...
import {leasesettleshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    leasesettleshow(params).then((res) => {

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-share"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>进场日期:</span><span>{{item.intodt}}</span></div>
            </div>
            <div class="list_s">
              <div><span>合同名称:</span><span>{{item.contract}}</span></div>
            </div>
            <div>
              <div><span>供应商名称:</span><span>{{item.supplier}}</span></div>
              <div><span>进场费合计(元):</span><span>{{item.cost}}</span></div>
            </div>
            <div>
            ...
            <div>
              <div><span>经办人:</span><span>{{item.person}}</span></div>
...
import {leaseintolist} from '../../../api/Api'
export default {
  name: 'Leaseinto',
  components: {
    NavBar,

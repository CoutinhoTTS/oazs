    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-circle-o-notch"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>所属合同:</span><span></span></div>
            </div>
            <div class="list_s">
              <div><span>供应商:</span><span>{{item.dealcom}}</span></div>
            </div>
            <div>
...
import {leasesettlelist} from '../../../api/Api'
export default {
  name: 'Leasesettle',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      leasesettlelist(params).then((res) => {

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-credit-card-alt"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>租赁单位:</span><span></span></div>
            </div>
            <div class="list_s">
              <div><span>付款名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>付款类型:</span><span></span></div>
...
import {leasepaymentlist} from '../../../api/Api'
export default {
  name: 'Leasepayment',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      leasepaymentlist(params).then((res) => {

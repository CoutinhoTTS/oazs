    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-reply"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>退场日期:</span><span>{{item.outdt}}</span></div>
            </div>
            <div class="list_s">
            ...
            <div>
              ...
              <div><span>退场费合计(元):</span><span>{{item.money}}</span></div>
            </div>
            <div>
...
import {leaseoutlist} from '../../../api/Api'
export default {
  name: 'Leaseout',
  components: {
    NavBar,

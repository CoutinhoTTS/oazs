    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-calculator"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>计划主题:</span><span>{{item.name}}</span></div>
            </div>
            <div>
...
import {leaseplanlist} from '../../../api/Api'
export default {
  name: 'Leaseplan',
  components: {
    NavBar,

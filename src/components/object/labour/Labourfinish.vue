    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-send-o"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
            ...
            <div>
              ...
              <div><span>劳务班组:</span><span></span></div>
            </div>
            <div>
...
import {labourfinishlist} from '../../../api/Api'
export default {
  name: 'Labourfinish',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      labourfinishlist(params).then((res) => {

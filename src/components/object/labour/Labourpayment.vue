    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-paypal"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>付款名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>付款类型:</span><span>{{item.paymethod}}</span></div>
              <div><span>付款金额(元):</span><span>{{item.paymoney}}</span></div>
            </div>
            <div>
              <div><span>劳务班组:</span><span>{{item.dealcom}}</span></div>
            </div>
            <div>
            ...
            <div>
              <div><span>经办人:</span><span>{{item.jingname}}</span></div>
...
import {labourpaymentlist} from '../../../api/Api'
export default {
  name: 'Labourpayment',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      labourpaymentlist(params).then((res) => {

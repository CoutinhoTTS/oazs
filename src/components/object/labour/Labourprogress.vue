    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-line-chart"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
            ...
            <div>
              ...
              <div><span>结束方式:</span><span>{{item.setmethod}}</span></div>
            </div>
            <div>
...
import {labourprogresslist} from '../../../api/Api'
export default {
  name: 'Labourprogress',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      labourprogresslist(params).then((res) => {

      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>申报名称:</div><div>{{list.usedt}}</div></div>
            <div><div>申报类型:</div><div>{{list.paymethod}}</div></div>
            <div><div>申报金额(元):</div><div>{{list.zename}}</div></div>
        ...
        <van-tab title="工程量清单">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              <div>
                <div><span>施工内容:</span><span>{{item.name}}</span></div>
              </div>
              <div>
                <div><span>工作内容:</span><span>{{item.unit}}</span></div>
              </div>
              <div>
                <div><span>价格(元):</span><span>{{item.price}}</span></div>
                <div><span>天:</span><span>{{item.day}}</span></div>
              </div>
              <div>
...
import {laboursporadicshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    laboursporadicshow(params).then((res) => {

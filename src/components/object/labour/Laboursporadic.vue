    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-puzzle-piece"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>名称:</span><span>{{item.name}}</span></div>
            </div>
            <div class="list_s">
              <div><span>用工日期:</span><span>{{item.usedt}}</span></div>
            </div>
            <div>
              <div><span>劳务费用(元):</span><span></span></div>
              <div><span>付款方式:</span><span>{{item.paymethod}}</span></div>
            </div>
            <div>
            ...
            <div>
              <div><span>责任人:</span><span>{{item.zename}}</span></div>
...
import {laboursporadiclist} from '../../../api/Api'
export default {
  name: 'Laboursporadic',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      laboursporadiclist(params).then((res) => {

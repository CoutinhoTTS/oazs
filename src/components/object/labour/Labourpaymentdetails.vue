  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>付款名称:</div><div>{{list.name}}</div></div>
        <div><div>劳务班组:</div><div>{{list.dealcom}}</div></div>
        <div><div>开户行名称:</div><div>{{list.openname}}</div></div>
        ...
        <div><div>付款类型:</div><div>{{list.paymethod}}</div></div>
        <div><div>付款金额(元):</div><div>{{list.paymoney}}</div></div>
...
import {labourpaymentshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    labourpaymentshow(params).then((res) => {

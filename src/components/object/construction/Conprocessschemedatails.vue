  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>方案名称:</div><div>{{list.name}}</div></div>
        <div><div>方案说明:</div><div>{{list.content}}</div></div>
...
import {conprocessschemeshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    conprocessschemeshow(params).then((res) => {

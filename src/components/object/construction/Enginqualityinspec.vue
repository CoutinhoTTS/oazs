    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-stethoscope"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>检查日期:</span><span>{{item.checkdt}}</span></div>
              <div><span>检查部位:</span><span>{{item.place}}</span></div>
            </div>
            <div>
            ...
            <div>
              <div><span>检查人:</span><span>{{item.checkname}}</span></div>

  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>整改日期:</div><div>{{list.checkdt}}</div></div>
        <div><div>整改部位:</div><div>{{list.place}}</div></div>
        <div><div>质量问题:</div><div>{{list.problem}}</div></div>
        <div><div>整改内容:</div><div>{{list.contents}}</div></div>
        <div><div>指派人员:</div><div>{{list.assignname}}</div></div>
        <div><div>指派日期:</div><div>{{list.assigndt}}</div></div>

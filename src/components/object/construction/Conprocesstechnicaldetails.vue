  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>技术交底名称:</div><div>{{list.name}}</div></div>
        <div><div>施工单位:</div><div>{{list.unit}}</div></div>
        <div><div>交底地点:</div><div>{{list.disaddress}}</div></div>
        <div><div>交底人:</div><div>{{list.disname}}</div></div>
        <div><div>被交底单位:</div><div>{{list.receunit}}</div></div>
        <div><div>被交底人:</div><div>{{list.receiver}}</div></div>
        <div><div>交底内容:</div><div>{{list.content}}</div></div>
...
import {conprocesstechnicalshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    conprocesstechnicalshow(params).then((res) => {

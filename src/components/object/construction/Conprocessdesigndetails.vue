  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>设计变更名称:</div><div>{{list.name}}</div></div>
        <div><div>提出变更单位:</div><div>{{list.changeunit}}</div></div>
        <div><div>变更前金额(元):</div><div>{{list.old_money}}</div></div>
        <div><div>变更后金额(元):</div><div>{{list.money}}</div></div>
        <div><div>变更原因:</div><div>{{list.reason}}</div></div>
        <div><div>变更内容:</div><div>{{list.content}}</div></div>
...
import {conprocessdesignshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    conprocessdesignshow(params).then((res) => {

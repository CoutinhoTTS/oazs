    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div>
              <div><i class="fa fa-star-half-o"></i><span>所属项目：{{item.engin}}</span></div>
              <div><span>所属合同:</span><span>{{item.contractname}}</span></div>
            </div>
            <div>
              <div><span>设计变更名称:</span><span>{{item.name}}</span></div>
              <div><span>提出变更单位:</span><span>{{item.changeunit}}</span></div>
            </div>
            <div>
              <div><span>变更后金额:</span><span>{{item.money}}</span></div>
            </div>
            <div>
...
import {conprocessdesignlist} from '../../../api/Api'
export default {
  ...
  methods: {
    ...
    onRefresh () {
      ...
      conprocessdesignlist(params).then((res) => {

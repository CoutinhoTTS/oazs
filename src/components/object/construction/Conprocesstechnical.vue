      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            ...
            <div class="list_s">
              <div><span>技术交底名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>交底人:</span><span>{{item.disname}}</span></div>
              <div><span>施工单位:</span><span>{{item.unit}}</span></div>
            </div>
            <div>
              <div><span>交底地点:</span><span>{{item.disaddress}}</span></div>
            </div>
            <div>
...
import {conprocesstechnicallist} from '../../../api/Api'
export default {
  ...
  methods: {
    ...
    onRefresh () {
      ...
      conprocesstechnicallist(params).then((res) => {

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-hospital-o"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>整改日期:</span><span>{{item.checkdt}}</span></div>
              <div><span>整改部位:</span><span>{{item.place}}</span></div>
            </div>
            <div class="list_s">
              <div><span>指派时间:</span><span>{{item.assigndt}}</span></div>
              <div><span>指派人员:</span><span>{{item.assignname}}</span></div>
            </div>
            <div>
...
import {enginqualitylist} from '../../../api/Api'
export default {
  ...
  methods: {
    ...
    onRefresh () {
      ...
      enginqualitylist(params).then((res) => {

      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            ...
            <div class="list_s">
              <div><span>采购日期:</span><span>{{item.dt}}</span></div>
              <div><span>采购员:</span><span>{{item.buy_name}}</span></div>
            </div>
            <div>
              <div><span>金额合计:</span><span>{{item.money}}</span></div>
...
import {deviceownbuylist} from '../../../api/Api'
export default {
  name: 'Deviceownbuy',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      deviceownbuylist(params).then((res) => {

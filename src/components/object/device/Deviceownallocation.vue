    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-refresh"></i><span>调拨日期：{{item.dt}}</span></div>
            </div>
            <div class="list_s">
              <div><span>原工程名称:</span><span>{{item.oldengin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>目标工程名称:</span><span>{{item.newengin}}</span></div>
            </div>
            <div>
...
import {deviceownallocationlist} from '../../../api/Api'
export default {
  name: 'Deviceownbuy',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      deviceownallocationlist(params).then((res) => {

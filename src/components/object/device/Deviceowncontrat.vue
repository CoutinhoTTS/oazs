    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-th-list"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>签订日期:</span><span>{{item.dt}}</span></div>
              <div><span>供应商名称:</span><span>{{item.supplier}}</span></div>
            </div>
            <div>
              <div><span>付款限额条件:</span><span>{{item.pay_condition}}</span></div>
              <div><span>是否结算:</span><span>{{item.is_pay}}</span></div>
            </div>
            <div>
              <div></div>
...
import {deviceowncontratlist} from '../../../api/Api'
export default {
  name: 'Deviceoil',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      deviceowncontratlist(params).then((res) => {

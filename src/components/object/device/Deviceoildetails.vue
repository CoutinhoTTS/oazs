      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>检查日期:</div><div>{{list.oildt}}</div></div>
        ...
        <van-tab title="设备检查明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                <div><span>油耗量:</span><span>{{item.oil}}</span></div>
                <div><span>油价(元):</span><span>{{item.oil_price}}</span></div>
              </div>
              <div>
                <div><span>油费(元):</span><span>{{item.oil_money}}</span></div>
              </div>
            </div>
          </div>
        </van-tab>
...
import {deviceoilshow, getreadsandlog} from '../../../api/Api'
export default {

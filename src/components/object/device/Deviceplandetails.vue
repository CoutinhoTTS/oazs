      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>抄送给谁:</div><div>{{list.name}}</div></div>
            <div><div>资金计划名称:</div><div>{{list.plandt}}</div></div>
          ...
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                ...
                <div><span>需用量:</span><span>{{item.expense}}</span></div>
              </div>
              <div>
...
import {deviceplanshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    deviceplanshow(params).then((res) => {

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-wrench"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>申请名称:</span><span>{{item.name}}</span></div>
              <div><span>报修类型:</span><span>{{item.type}}</span></div>
            </div>
            <div>
              <div><span>报修人:</span><span>{{item.username}}</span></div>
              <div><span>申请时间:</span><span>{{item.applydt}}</span></div>
            </div>
            <div>
              <div></div>
...
import {devicerepairlist} from '../../../api/Api'
export default {
  name: 'Deviceoil',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      devicerepairlist(params).then((res) => {

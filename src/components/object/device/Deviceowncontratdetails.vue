      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>检查名称:</div><div>{{list.dt}}</div></div>
            <div><div>检查日期:</div><div>{{list.supplier}}</div></div>
            <div><div>检查日期:</div><div>{{list.total_money}}</div></div>
            <div><div>检查日期:</div><div>{{list.pay_money}}</div></div>
            <div><div>检查日期:</div><div>{{list.pay_condition}}</div></div>
            <div><div>检查日期:</div><div>{{list.pay_scale}}</div></div>
            <div><div>检查日期:</div><div>{{list.is_pay}}</div></div>
        ...
        <van-tab title="设备明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              <div>
                <div><span>设备编号:</span><span>{{item.number}}</span></div>
              ...
              <div>
                <div><span>生产厂家:</span><span>{{item.product}}</span></div>
              </div>
              <div>
              ...
              <div>
                <div><span>金额(元):</span><span>{{item.money}}</span></div>
              </div>
              <div>
...
import {deviceowncontratshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    deviceowncontratshow(params).then((res) => {

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-ioxhost"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>计划名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>计划日期:</span><span>{{item.plandt}}</span></div>
...
import {deviceplanlist} from '../../../api/Api'
export default {
  name: 'Deviceplan',
  components: {
    NavBar,

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-pie-chart"></i><span>工程名称：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>报废日期:</span><span>{{item.dt}}</span></div>
              <div><span>施工部位:</span><span>{{item.position}}</span></div>
            </div>
            <div>
              <div><span>经办人:</span><span>{{item.jingban}}</span></div>
              <div><span>报废总残值:</span><span>{{item.money}}</span></div>
            </div>
            <div>
              <div><span>申请人:</span><span>{{item.realname}}</span></div>
...
import {deviceownscraplist} from '../../../api/Api'
export default {
  name: 'Deviceownbuy',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      deviceownscraplist(params).then((res) => {

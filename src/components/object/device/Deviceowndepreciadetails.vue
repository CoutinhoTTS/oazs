      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>工程名称:</div><div>{{list.enginname}}</div></div>
            <div><div>折旧日期:</div><div>{{list.dt}}</div></div>
            <div><div>施工部位:</div><div>{{list.position}}</div></div>
            <div><div>折旧总金额:</div><div>{{list.money}}</div></div>
        ...
        <van-tab title="设备明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                <div><span>出场单号:</span><span>{{item.out_num}}</span></div>
              </div>
              <div>
              ...
              <div>
                ...
                <div><span>结束日期:</span><span>{{item.enddt}}</span></div>
              </div>
              <div>
                <div><span>计价方式:</span><span>{{item.price_type}}</span></div>
                <div><span>单价(元):</span><span>{{item.price}}</span></div>
              </div>
              <div>
                <div><span>工作时间:</span><span>{{item.work_time}}</span></div>
                <div><span>折旧金额(元):</span><span>{{item.money}}</span></div>
              </div>
              <div>
...
import {deviceowndepreciashow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    deviceowndepreciashow(params).then((res) => {

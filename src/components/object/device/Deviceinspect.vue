    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-user-md"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>检查名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>检查日期:</span><span>{{item.inspectdt}}</span></div>
...
import {deviceinspectlist} from '../../../api/Api'
export default {
  name: 'Deviceinspect',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      deviceinspectlist(params).then((res) => {

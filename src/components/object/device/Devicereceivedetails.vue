  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>设备名称:</div><div>{{list.name}}</div></div>
        <div><div>使用时长:</div><div>{{list.duration}}</div></div>
        <div><div>使用日期:</div><div>{{list.usedt}}</div></div>
        <div><div>紧急程度:</div><div>{{list.urgent}}</div></div>
        <div><div>使用类型:</div><div>{{list.usetype}}</div></div>
        <div><div>申请原因:</div><div>{{list.reason}}</div></div>
        <div><div>配置要求:</div><div>{{list.configure}}</div></div>
...
import {devicereceiveshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    devicereceiveshow(params).then((res) => {

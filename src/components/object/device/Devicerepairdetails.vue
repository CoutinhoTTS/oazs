      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>检查日期:</div><div>{{list.username}}</div></div>
            <div><div>检查日期:</div><div>{{list.userdept}}</div></div>
            <div><div>检查日期:</div><div v-if="list.type === '1'">维修</div><div v-else>维保</div></div>
          ...
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                ...
                <div><span>维修内容:</span><span>{{item.content}}</span></div>
              </div>
              <div>
                <div><span>故障程度:</span><span>{{item.fault}}</span></div>
                <div><span>人工费(元):</span><span>{{item.person_cost}}</span></div>
              </div>
              <div>
                <div><span>材料费(元):</span><span>{{item.stuff_cost}}</span></div>
                <div><span>停工天数(天):</span><span>{{item.stoppage}}</span></div>
              </div>
              <div>
...
import {devicerepairshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    devicerepairshow(params).then((res) => {

      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>采购日期:</div><div>{{list.dt}}</div></div>
            <div><div>采购员:</div><div>{{list.buy_name}}</div></div>
            <div><div>供应商名称:</div><div>{{list.supplier}}</div></div>
            <div><div>单据编号:</div><div>{{list.number}}</div></div>
            <div><div>金额合计:</div><div>{{list.money}}</div></div>
            <div><div>已付款比例:</div><div>{{list.scale}}</div></div>
          ...
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                ...
                <div><span>直接计入成本:</span><span>{{item.prime_cost}}</span></div>
              </div>
              <div>
...
import {deviceownbuyshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    deviceownbuyshow(params).then((res) => {

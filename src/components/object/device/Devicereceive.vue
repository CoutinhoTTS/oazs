    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-outdent"></i><span>设备名称：{{item.name}}</span></div>
            </div>
            <div class="list_s">
              <div><span>使用时长(月):</span><span>{{item.duration}}</span></div>
            </div>
            <div>
              <div><span>使用日期:</span><span>{{item.usedt}}</span></div>
              <div><span>紧急程度:</span><span>{{item.urgent}}</span></div>
            </div>
            <div>
              <div><span>使用类型:</span><span>{{item.usetype}}</span></div>
...
import {devicereceivelist} from '../../../api/Api'
export default {
  name: 'Devicereceive',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      devicereceivelist(params).then((res) => {

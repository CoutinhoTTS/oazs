      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>检查日期:</div><div>{{list.position}}</div></div>
            <div><div>检查日期:</div><div>{{list.jingban}}</div></div>
            <div><div>检查日期:</div><div>{{list.money}}</div></div>
          ...
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                ...
                <div><span>残值单价:</span><span>{{item.scrap_unit}}</span></div>
              </div>
              <div>
                <div><span>数量:</span><span>{{item.amount}}</span></div>
                <div><span>设备残值:</span><span>{{item.money}}</span></div>
              </div>
              <div>
                <div><span>设计成本:</span><span v-if="item.prime_cost === '0'">否</span><span v-else>是</span></div>
              </div>
              <div>
...
import {deviceownscrapshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    deviceownscrapshow(params).then((res) => {

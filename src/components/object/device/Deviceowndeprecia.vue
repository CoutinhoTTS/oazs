    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-gears"></i><span>工程名称：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>折旧日期:</span><span>{{item.dt}}</span></div>
            </div>
            <div>
              <div><span>施工部位:</span><span>{{item.position}}</span></div>
              <div><span>折旧总金额:</span><span>{{item.money}}</span></div>
            </div>
            <div>
...
import {deviceowndeprecialist} from '../../../api/Api'
export default {
  name: 'Deviceinspect',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      deviceowndeprecialist(params).then((res) => {

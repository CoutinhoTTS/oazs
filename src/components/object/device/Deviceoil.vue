    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-thermometer-empty"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>日期:</span><span>{{item.oildt}}</span></div>
...
import {deviceoillist} from '../../../api/Api'
export default {
  name: 'Deviceoil',
  components: {
    NavBar,

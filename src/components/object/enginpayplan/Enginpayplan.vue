    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-sign-out"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>资金计划名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>计划月份:</span><span>{{item.month}}</span></div>
              <div><span>申请金额(元):</span><span>{{item.money}}</span></div>
            </div>
            <div>
...
import {enginpayplanlist} from '../../../api/Api'
export default {
  name: 'Enginpayplan',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      enginpayplanlist(params).then((res) => {

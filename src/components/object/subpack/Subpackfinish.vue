    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-folder-open-o"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>所属项目:</span><span>{{item.contract}}</span></div>
            </div>
            <div class="list_s">
              <div><span>分包单位:</span><span></span></div>
            </div>
            <div class="list_s">
              <div><span>结算名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>结算金额(元):</span><span>{{item.setmoney}}</span></div>
            </div>
            <div>
...
import {subpackfinishlist} from '../../../api/Api'
export default {
  name: 'Subpackfinish',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      subpackfinishlist(params).then((res) => {

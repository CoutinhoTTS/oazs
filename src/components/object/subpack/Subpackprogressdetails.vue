        ...
        <van-tab title="分包清单明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                <div><span>结算小计(元):</span><span>{{item.subtotal}}</span></div>
              </div>
            </div>
          </div>
        </van-tab>
...
import {subpackprogressshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    subpackprogressshow(params).then((res) => {
      console.log(res)

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-exchange"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
            ...
            <div>
              <div><span>申报名称:</span><span>{{item.name}}</span></div>
              <div><span>申报金额(元):</span><span>{{item.applymoney}}</span></div>
            </div>
            <div>
...
import {subpackprogresslist} from '../../../api/Api'
export default {
  ...
  methods: {
    ...
    onRefresh () {
      ...
      subpackprogresslist(params).then((res) => {

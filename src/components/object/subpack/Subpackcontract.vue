    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-folder-o"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>合同名称:</span><span>{{item.name}}</span></div>
            </div>
            <div class="list_s">
              <div><span>分包单位:</span><span>{{item.packname}}</span></div>
            </div>
            <div class="list_s">
              <div><span>合同起止时间:</span><span>{{item.dt}}</span></div>
            </div>
            <div>
            ...
            <div>
              ...
              <div><span>签约人:</span><span>{{item.qianname}}</span></div>
            </div>
            <div>
...
import {subpackcontractlis} from '../../../api/Api'
export default {
  ...
  methods: {
    ...
    onRefresh () {
      ...
      subpackcontractlis(params).then((res) => {

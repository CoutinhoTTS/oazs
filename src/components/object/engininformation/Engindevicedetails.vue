  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        <div><div>设备编号:</div><div>{{list.number}}</div></div>
        <div><div>设备名称:</div><div>{{list.name}}</div></div>
        <div><div>设备型号:</div><div>{{list.marking}}</div></div>
        <div><div>单位:</div><div>{{list.unit}}</div></div>
        <div><div>生产厂家:</div><div>{{list.factory}}</div></div>
        <div><div>价格:</div><div>{{list.price}}</div></div>
        <div><div>供应商:</div><div>{{list.customer}}</div></div>
      </div>
      <!--文件-->
      <File ref="files"></File>
      <!--审核，游览-->
...
import {engindeviceshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    engindeviceshow(params).then((res) => {

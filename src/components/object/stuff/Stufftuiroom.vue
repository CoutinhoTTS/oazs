    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-reply-all"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>退库名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>退库仓库:</span><span>{{item.depot}}</span></div>
              <div><span>退库班组:</span><span>{{item.allocs}}</span></div>
            </div>
            <div>
              <div><span>退库时间:</span><span>{{item.dt}}</span></div>
              <div><span>经办人:</span><span>{{item.chuname}}</span></div>
            </div>
            <div>
...
import {stufftuiroomlist} from '../../../api/Api'
export default {
  name: 'Stufftuiroom',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      stufftuiroomlist(params).then((res) => {

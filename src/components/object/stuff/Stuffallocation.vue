    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-pencil-square-o"></i><span>调拨名称：{{item.name}}</span></div>
            </div>
            <div class="list_s">
              <div><span>调拨日期:</span><span>{{item.dt}}</span></div>
            </div>
            <div>
              <div><span>调出项目:</span><span>{{item.out_engin}}</span></div>
              <div><span>调出仓库:</span><span>{{item.out_depot}}</span></div>
            </div>
            <div>
              <div><span>调入项目:</span><span>{{item.into_engin}}</span></div>
              <div><span>调入仓库:</span><span>{{item.into_depot}}</span></div>
            </div>
            <div>
            ...
            <div>
              <div><span>经办人:</span><span></span></div>
...
import {stuffallocationlist} from '../../../api/Api'
export default {
  ...
  methods: {
    ...
    onRefresh () {
      ...
      stuffallocationlist(params).then((res) => {

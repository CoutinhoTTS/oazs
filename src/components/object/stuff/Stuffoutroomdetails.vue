      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>出库主题:</div><div>{{list.name}}</div></div>
            <div><div>出库人名:</div><div>{{list.chuname}}</div></div>
            <div><div>仓库:</div><div>{{list.depot}}</div></div>
            <div><div>领用人名:</div><div>{{list.usename}}</div></div>
            <div><div>入库人名称:</div><div>{{list.chuname}}</div></div>
            <div><div>领用时间:</div><div>{{list.usedt}}</div></div>
            <div><div>出库原因:</div><div>{{list.outreason}}</div></div>
...
import {stuffoutroomshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    stuffoutroomshow(params).then((res) => {

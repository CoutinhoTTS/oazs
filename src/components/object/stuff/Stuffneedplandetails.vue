      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>计划名称:</div><div>{{list.name}}</div></div>
        ...
        <van-tab title="需用明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                <div><span>需用数量:</span><span>{{item.need_number}}</span></div>
              </div>
              <div>
                <div><span>参考价格(元):</span><span>{{item.refprice}}</span></div>
              </div>
              <div>
                <div><span>小计(元):</span><span>{{item.subtotal}}</span></div>
              </div>
              <div>
                <div><span>备用:</span><span>{{item.remark}}</span></div>
              </div>
            </div>
          </div>
        </van-tab>
...
import {stuffplanshow, getreadsandlog} from '../../../api/Api'
export default {

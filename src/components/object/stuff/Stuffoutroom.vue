    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-arrow-circle-o-left"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>出库主题:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>出库人:</span><span>{{item.chuname}}</span></div>
              <div><span>仓库:</span><span>{{item.depot}}</span></div>
            </div>
            <div>
              <div><span>领用人:</span><span>{{item.usename}}</span></div>
              <div><span>领用时间:</span><span>{{item.usedt}}</span></div>
            </div>
            <div>
...
import {stuffoutroomlist} from '../../../api/Api'
export default {
  name: 'Stuffoutroom',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      stuffoutroomlist(params).then((res) => {

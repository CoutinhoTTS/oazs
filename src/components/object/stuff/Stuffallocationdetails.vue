      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>调拨名称:</div><div>{{list.name}}</div></div>
            <div><div>调拨日期:</div><div>{{list.dt}}</div></div>
            <div><div>调出项目:</div><div>{{list.out_enginname}}</div></div>
            <div><div>调出仓库:</div><div>{{list.out_depot}}</div></div>
            <div><div>调入项目:</div><div>{{list.into_enginname}}</div></div>
            <div><div>调入仓库:</div><div>{{list.into_depot}}</div></div>
        ...
        <van-tab title="材料明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                <div><span>调拨数量:</span><span>{{item.allocation_num}}</span></div>
              </div>
              <div>
...
import {stuffallocationshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    stuffallocationshow(params).then((res) => {

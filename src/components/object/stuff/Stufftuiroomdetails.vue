      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>退库名称:</div><div>{{list.name}}</div></div>
            <div><div>退库仓库:</div><div>{{list.depot}}</div></div>
            <div><div>录入人:</div><div>{{list.chuname}}</div></div>
            <div><div>dt:</div><div>{{list.dt}}</div></div>
            <div><div>退库班组:</div><div>{{list.allocs}}</div></div>
        ...
        <van-tab title="出库明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                <div><span>退库数量:</span><span>{{item.tui_number}}</span></div>
              </div>
              <div>
...
import {stufftuiroomshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    stufftuiroomshow(params).then((res) => {

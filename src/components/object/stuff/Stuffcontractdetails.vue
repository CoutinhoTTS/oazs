        ...
        <van-tab title="材料明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                <div><span>采购数量:</span><span>{{item.cai_number}}</span></div>
              </div>
              <div>
                <div><span>采购价格(元):</span><span>{{item.cai_money}}</span></div>
              </div>
              <div>
...
import {stuffcontractshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    stuffcontractshow(params).then((res) => {

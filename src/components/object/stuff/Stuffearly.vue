    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-hourglass-o"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>入库主题:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>入库日期:</span><span>{{item.dt}}</span></div>
...
import {stuffearlylist} from '../../../api/Api'
export default {
  name: 'Stuffearly',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      stuffearlylist(params).then((res) => {

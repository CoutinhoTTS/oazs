      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>所属项目:</div><div>{{list.enginname}}</div></div>
            <div><div>入库主题:</div><div>{{list.name}}</div></div>
            <div><div>入库日期:</div><div>{{list.dt}}</div></div>
...
import {stuffearlyshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    stuffearlyshow(params).then((res) => {

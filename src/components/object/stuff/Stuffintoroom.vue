    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-arrow-circle-o-right"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
            ...
            <div>
              ...
              <div><span>入库人:</span><span>{{item.runame}}</span></div>
            </div>
            <div>
...
import {stuffintoroomlist} from '../../../api/Api'
export default {
  name: 'Stuffintoroom',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      stuffintoroomlist(params).then((res) => {

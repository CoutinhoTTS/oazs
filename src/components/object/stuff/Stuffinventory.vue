      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            ...
            <div class="list_s">
              <div><span>盘点名称:</span><span>{{item.name}}</span></div>
              <div><span>盘点仓库:</span><span>{{item.depotname}}</span></div>
            </div>
            <div>
              <div><span>盘点人:</span><span>{{item.person}}</span></div>
              <div><span>盘点时间:</span><span>{{item.dt}}</span></div>
            </div>
            <div>
...
import {stuffinventorylist} from '../../../api/Api'
export default {
  name: 'Stuffinventory',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      stuffinventorylist(params).then((res) => {

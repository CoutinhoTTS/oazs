  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>付款名称:</div><div>{{list.enginname}}</div></div>
        <div><div>未付款:</div><div>{{list.name}}</div></div>
        <div><div>付款金额(元):</div><div>{{list.notmoney}}</div></div>
...
import {stuffpaymentshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    stuffpaymentshow(params).then((res) => {

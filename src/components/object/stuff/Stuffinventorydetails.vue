      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>盘点名称:</div><div>{{list.name}}</div></div>
            <div><div>盘点仓库:</div><div>{{list.depotname}}</div></div>
            <div><div>盘点人:</div><div>{{list.person}}</div></div>
            <div><div>盘点时间:</div><div>{{list.dt}}</div></div>
        ...
        <van-tab title="材料明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                <div><span>仓库:</span><span>{{item.account_number}}</span></div>
              </div>
              <div>
                <div><span>入库数量:</span><span>{{item.inventory_number}}</span></div>
              </div>
            </div>
          </div>
        </van-tab>
...
import {stuffinventoryshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    stuffinventoryshow(params).then((res) => {

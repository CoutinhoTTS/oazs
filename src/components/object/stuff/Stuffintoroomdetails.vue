      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>入库主题:</div><div>{{list.name}}</div></div>
            <div><div>采购员名称:</div><div>{{list.cainame}}</div></div>
            <div><div>交货人:</div><div>{{list.jiaoname}}</div></div>
            <div><div>验收人名称:</div><div>{{list.yanname}}</div></div>
            <div><div>入库人名称:</div><div>{{list.runame}}</div></div>
        ...
        <van-tab title="入库明细">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              <div>
                <div><span>材料名称:</span><span>{{item.name}}</span></div>
              </div>
              <div>
              ...
              <div>
                <div><span>仓库:</span><span>{{item.depot}}</span></div>
              </div>
              <div>
                <div><span>入库数量:</span><span>{{item.into_number}}</span></div>
              </div>
              <div>
                <div><span>不含税单价(元):</span><span>{{item.nottaxprice}}</span></div>
                <div><span>含税单价(元):</span><span>{{item.taxprice}}</span></div>
              </div>
              <div>
                <div><span>税率:</span><span>{{item.rate}}</span></div>
              </div>
              <div>
                <div><span>含税金额(元):</span><span>{{item.taxamount}}</span></div>
                <div><span>不含税金额(元):</span><span>{{item.nottaxamount}}</span></div>
              </div>
            </div>
          </div>
        </van-tab>
...
import {stuffintoroomshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    stuffintoroomshow(params).then((res) => {

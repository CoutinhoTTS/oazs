        ...
        <van-tab title="工程量清单">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                <div><span>合同数量:</span><span>{{item.henum}}</span></div>
              </div>
              <div>
                <div><span>合同单价:</span><span>{{item.heprice}}</span></div>
              </div>
              <div>
                <div><span>实际完成数量:</span><span>{{item.shifinish}}</span></div>
              </div>
              <div>
                <div><span>实际执行价:</span><span>{{item.shiprice}}</span></div>
              </div>
              <div>
                <div><span>结算价款:</span><span>{{item.setmoney}}</span></div>
              </div>
              <div>
...
import {incomefinishshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    incomefinishshow(params).then((res) => {

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-retweet"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>所属合同:</span><span>{{item.contract}}</span></div>
            </div>
            <div>
              <div><span>变更名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>合同原金额(元):</span><span>{{item.contractmoney}}</span></div>
              <div><span>变更金额(元):</span><span>{{item.money}}</span></div>
            </div>
            <div>
              <div><span>变更签订人:</span><span>{{item.qianname}}</span></div>
...
import {incomemodifylist} from '../../../api/Api'
export default {
  name: 'Incomemodify',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      incomemodifylist(params).then((res) => {

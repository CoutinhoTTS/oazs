  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>收款名称:</div><div>{{list.name}}</div></div>
        <div><div>收款类型:</div><div>{{list.colmethod}}</div></div>
        <div><div>收款金额(元):</div><div>{{list.money}}</div></div>
        <div><div>账户信息:</div><div>{{list.bankname}}</div></div>
...
import {incomecollectshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    incomecollectshow(params).then((res) => {

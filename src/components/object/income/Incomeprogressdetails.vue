      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>申报名称:</div><div>{{list.setmethod}}</div></div>
        ...
        <van-tab title="申报内容">
          <div class="conlists">
            <div v-for="(item, index) in conlist" :key="index">
              ...
              <div>
                <div><span>清单子目录:</span><span>{{item.name}}</span></div>
              </div>
              <div>
              ...
              <div>
                <div><span>本期申请量:</span><span>{{item.apply_number}}</span></div>
              </div>
              <div>
                <div><span>本期核准量:</span><span>{{item.verify_number}}</span></div>
              </div>
              <div>
                <div><span>结算小结:</span><span>{{item.subtotal}}</span></div>
              </div>
            </div>
          </div>
        </van-tab>
...
import {incomeprogressshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    incomeprogressshow(params).then((res) => {

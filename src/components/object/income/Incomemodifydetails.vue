      ...
      <van-tabs v-model="active" swipeable :sticky="issticky" :offset-top="isoffsettop">
        <van-tab title="基本信息">
          <div class="datailsList">
            ...
            <div><div>变更名称:</div><div>{{list.name}}</div></div>
            <div><div>变更金额(元):</div><div>{{list.money}}</div></div>
            <div><div>变更签订人:</div><div>{{list.qianname}}</div></div>
            <div><div>结算方式:</div><div>{{list.setmethod_name}}</div></div>
...
import {incomemodifyshow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    incomemodifyshow(params).then((res) => {

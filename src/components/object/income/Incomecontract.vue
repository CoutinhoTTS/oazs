    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-sign-in"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
            ...
            <div>
              <div><span>日期:</span><span>{{item.dt}}</span></div>
            </div>
            <div>
              <div><span>签订人:</span><span>{{item.qianname}}</span></div>
              <div><span>合同金额(万元):</span><span>{{item.money}}</span></div>
            </div>
            <div>
              <div><span>甲方单位:</span><span>{{item.onename}}</span></div>
              <div><span>乙方单位:</span><span>{{item.twoname}}</span></div>
            </div>
            <div>
...
import {incomecontractlist} from '../../../api/Api'
export default {
  name: 'Incomecontract',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      incomecontractlist(params).then((res) => {

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-file-word-o"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>收款合同:</span><span>{{item.contract}}</span></div>
            </div>
            <div>
              <div><span>收款名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>收款类型:</span><span>{{item.colmethod}}</span></div>
            </div>
            <div>
              <div><span>收款金额(元):</span><span>{{item.money}}</span></div>
...
import {incomecollectlist} from '../../../api/Api'
export default {
  name: 'Incomecollect',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      incomecollectlist(params).then((res) => {

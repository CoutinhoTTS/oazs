    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div>
              <div><i class="fa fa-print"></i><span>所属项目：{{item.engin}}</span></div>
              <div><span>填报名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>填报人:</span><span>{{item.realname}}</span></div>
              <div><span>填报日期:</span><span>{{item.applydt}}</span></div>
            </div>
            <div>
              <div></div>
...
import {enginspeedlist} from '../../../api/Api'
export default {
  name: 'Enginspeed',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      enginspeedlist(params).then((res) => {

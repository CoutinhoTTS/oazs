  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>所属项目:</div><div>{{list.project_title}}</div></div>
        <div><div>任务等级:</div><div>{{list.grade}}</div></div>
        <div><div>任务标题:</div><div>{{list.title}}</div></div>
        <div><div>分配给:</div><div>{{list.dist}}</div></div>
        <div><div>开始时间:</div><div>{{list.startdt}}</div></div>
        <div><div>截止时间:</div><div>{{list.enddt}}</div></div>
        <div><div>督导人:</div><div>{{list.ddname}}</div></div>
        <div><div>申请人:</div><div>{{list.optname}}</div></div>
        <div><div>申请人部门:</div><div>{{list.optdt}}</div></div>
      </div>
      <van-panel title="说明:" >
...
import {workshow, getreadsandlog} from '../../../api/Api'
export default {
  name: 'Workmydetails',
  components: {
    NavBar,

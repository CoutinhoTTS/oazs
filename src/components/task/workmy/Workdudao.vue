    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div>
              <div><i class="fa fa-wordpress"></i><span>标题：{{item.title}}</span></div>
            </div>
            <div class="list_s">
              <div><span>所属项目:</span><span>{{item.project}}</span></div>
            </div>
            <div>
              <div><span>分配给:</span><span>{{item.dist}}</span></div>
            ...
            <div>
              ...
              <div><span>创建时间:</span><span>{{item.optdt}}</span></div>
            </div>
            <div>
              <div></div>
              <div v-html="item.statue_name">
              </div>
            </div>
          </div>
        </div>
        ...
        if (res.data.code === 2000) {
          if (this.page === 1) {
            ...
            for (var j = 0; j < this.status.length; j++) {
              this.columns.push(
                {
                  text: this.status[j].name,
                  value: this.status[j].id
                }
              )
            }
          }

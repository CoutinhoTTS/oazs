    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-ship"></i><span>客户名称：{{item.name}}</span></div>
            </div>
            <div>
            ...
            <div>
              <div></div>
              <div><span>状态:</span><span v-if="item.status === 0" style="color: green">已停用</span><span v-else style="color: green">已启用</span></div>
            </div>
          </div>
        </div>
      </van-list>

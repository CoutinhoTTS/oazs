  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        <div><div>账户名称:</div><div>{{list.bankname}}</div></div>
        <div><div>开户银行:</div><div>{{list.openname}}</div></div>
        <div><div>账号:</div><div>{{list.bankaccount}}</div></div>
        <div><div>地址:</div><div>{{list.address}}</div></div>
        <div><div>开户日期:</div><div>{{list.opendt}}</div></div>
        <div><div>经办人:</div><div>{{list.jingname}}</div></div>
        <div><div>现金账户:</div><div>{{list.iscash}}</div></div>
        <div><div>状态:</div><div>{{list.status}}</div></div>
        <div><div>账户类别:</div><div>{{list.type}}</div></div>
      </div>
      <van-panel title="说明:" >
...
import {bankshow, getreadsandlog} from '../../../api/Api'
export default {

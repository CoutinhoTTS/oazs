    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-credit-card"></i><span>所属项目：{{item.engin}}</span></div>
            </div>
            <div class="list_s">
              <div><span>分包单位:</span><span></span></div>
            </div>
            <div class="list_s">
...
import {subpackpaymentlist} from '../../../api/Api'
export default {
  ...
  methods: {
    ...
    onRefresh () {
      ...
      subpackpaymentlist(params).then((res) => {

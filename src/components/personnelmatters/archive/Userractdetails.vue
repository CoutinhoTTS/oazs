  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        <div><div>签署人:</div><div>{{list.uname}}</div></div>
        <div><div>签署单位:</div><div>{{list.company}}</div></div>
        <div><div>合同名称:</div><div>{{list.name}}</div></div>
        <div><div>合同类型:</div><div>{{list.httype}}</div></div>
        <div><div>开始时间:</div><div>{{list.startdt}}</div></div>
        <div><div>截止日期:</div><div>{{list.enddt}}</div></div>
        <div><div>提取终止日期:</div><div>{{list.tqenddt}}</div></div>
        <div><div>状态:</div><div>{{list.state_name}}</div></div>
      </div>
      <van-panel title="说明:" >
...
import {userractshow, getreadsandlog} from '../../../api/Api'
export default {

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-arrow-circle-o-right"></i><span>工作类别：{{item.type}}</span></div>
            </div>
            <div>
              <div><span>单位名称:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>工资月份:</span><span>{{item.month}}</span></div>
            </div>
            <div>
              <div><span>工资单号:</span><span>{{item.number}}</span></div>
              <div><span>本月工作日(天):</span><span>{{item.monthday}}</span></div>
            </div>
            <div>
              <div><span>发放日期:</span><span>{{item.grantdt}}</span></div>
              <div><span>发放人数:</span><span>{{item.grantperson}}</span></div>
            </div>
            <div>
              <div><span>发放工资(元):</span><span>{{item.grantwages}}</span></div>
              <div><span>发放方式:</span><span>{{item.granttype}}</span></div>
            </div>
            <div>
              <div></div>

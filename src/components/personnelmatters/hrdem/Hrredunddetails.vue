  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>入职日期:</div><div>{{list.entrydt}}</div></div>
        <div><div>离职类型:</div><div>{{list.redundtype}}</div></div>
        <div><div>离职日期:</div><div>{{list.quitdt}}</div></div>
        <div><div>离职原因:</div><div>{{list.redundreson}}</div></div>
      </div>
      <van-panel title="说明:" >
...
import {hrredundshow, getreadsandlog} from '../../../api/Api'
export default {

    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-user-circle"></i><span>申请人：{{item.realname}}</span></div>
            </div>
            <div>
              <div><span>申请人部门:</span><span>{{item.deptname}}</span></div>
              <div><span>面试者:</span><span>{{item.name}}</span></div>
            </div>
            <div>
              <div><span>面试岗位:</span><span>{{item.zhiwei}}</span></div>
              <div><span>需求部门:</span><span>{{item.bumen}}</span></div>
            </div>
            <div>
              <div><span>面试时间:</span><span>{{item.msdt}}</span></div>
              <div><span>面试人员:</span><span>{{item.msuser}}</span></div>
            </div>
            <div>
              <div><span>面试结果:</span><span>{{item.state_name}}</span></div>
...
import {hrdemintviewlist} from '../../../api/Api'
export default {
  name: 'Hrdemintview',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      hrdemintviewlist(params).then((res) => {

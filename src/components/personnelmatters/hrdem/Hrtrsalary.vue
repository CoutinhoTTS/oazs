    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-hand-o-up"></i><span>申请人：{{item.realname}}</span></div>
            </div>
            <div>
              <div><span>申请人部门:</span><span>{{item.dept_name}}</span></div>
              <div><span>申请日期:</span><span>{{item.applydt}}</span></div>
            </div>
            <div>
              <div><span>职位:</span><span>{{item.ranking}}</span></div>
              <div><span>调薪幅度:</span><span>{{item.floats}}</span></div>
            </div>
            <div>
...
import {hrtrsalarylist} from '../../../api/Api'
export default {
  name: 'Hrtrsalary',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      hrtrsalarylist(params).then((res) => {

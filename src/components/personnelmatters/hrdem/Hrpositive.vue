    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="list">
      ...
      >
        <div v-for="(item, index) in list" class="lists" :key="index" @click="engindetails(item.id)" >
          <div>
            <div class="list_s">
              <div><i class="fa fa-user-plus"></i><span>申请人：{{item.realname}}</span></div>
            </div>
            <div>
            ...
            <div>
              <div><span>试用到期日:</span><span>{{item.syenddt}}</span></div>
              <div><span>转正日期:</span><span>{{item.positivedt}}</span></div>
            </div>
            <div>
              <div></div>
...
import {hrpositivelist} from '../../../api/Api'
export default {
  name: 'Hrpositive',
  components: {
    NavBar,
  ...
  methods: {
    ...
    onRefresh () {
      ...
      hrpositivelist(params).then((res) => {

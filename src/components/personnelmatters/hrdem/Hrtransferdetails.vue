  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>要调动人:</div><div>{{list.tranname}}</div></div>
        <div><div>原来部门:</div><div>{{list.olddeptname}}</div></div>
        <div><div>原来职位:</div><div>{{list.oldranking}}</div></div>
        <div><div>调动后部门:</div><div>{{list.newdeptname}}</div></div>
        <div><div>调动后职位:</div><div>{{list.newranking}}</div></div>
        <div><div>生效日期:</div><div>{{list.effectivedt}}</div></div>
        <div><div>调动类型:</div><div>{{list.trantype}}</div></div>
      </div>
      <van-panel title="说明:" >
...
import {hrtransfershow, getreadsandlog} from '../../../api/Api'
export default {
  ...
  created () {
    ...
    hrtransfershow(params).then((res) => {

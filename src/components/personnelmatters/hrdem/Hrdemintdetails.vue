  ...
  <div>
    ...
    <van-pull-refresh v-model="isLoading_" @refresh="onRefresh" class="datails_pull">
      <div class="datailsList">
        ...
        <div><div>申请人部门:</div><div>{{list.udeptname}}</div></div>
        <div><div>申请日期:</div><div>{{list.applydt}}</div></div>
        <div><div>需求岗位:</div><div>{{list.zhiwei}}</div></div>
        <div><div>需求部门:</div><div>{{list.bumen}}</div></div>
        <div><div>薪资:</div><div>{{list.xinzi}}</div></div>
        <div><div>需求人数:</div><div>{{list.renshu}}</div></div>
        <div><div>职位要求:</div><div>{{list.content}}</div></div>
      </div>
      <van-panel title="说明:" >
...
import {hrdemintshow, getreadsandlog} from '../../../api/Api'
export default {
